...
String currentMessage = "";               //Variables to keep track of time and free memory
...
void runEncryption(unsigned char* payload) {
   startEncryptTime = micros();
   blowfishObject.Encode(payload, encryptedData, 16);  //Encrypt the char array
...
void runDecryption(unsigned char* payload) {
   startDecryptTime = micros();
   blowfishObject.Decode(payload, decryptedData, 16); //Encrypt the char array
