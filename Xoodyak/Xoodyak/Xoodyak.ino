...
void loop() {
  ...
  if(j < 200) {
    ...
    startTime = micros();
    xoodyak_aead_encrypt(encryptedData, &sizeOfEncrypted, messageMemory, sizeof(messageMemory), authMemory, 16, IV, keyMemory);  
    ...
    startTime = micros();
    authenticated = xoodyak_aead_decrypt(decryptedData, &sizeOfDecrypted, encryptedData, sizeof(encryptedData), authMemory, 16, IV, keyMemory);
    totalTime = micros() - startTime;
